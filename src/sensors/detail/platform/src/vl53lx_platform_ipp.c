...
#define LOG_FUNCTION_START(fmt, ...)  _LOG_FUNCTION_START(VL53LX_TRACE_MODULE_CORE, fmt, ##__VA_ARGS__)
#define LOG_FUNCTION_END(status, ...) _LOG_FUNCTION_END(VL53LX_TRACE_MODULE_CORE, status, ##__VA_ARGS__)


VL53LX_Error VL53LX_ipp_hist_process_data(VL53LX_DEV Dev,
...
VL53LX_Error VL53LX_ipp_hist_ambient_dmax(VL53LX_DEV Dev,
                                          ...
                                          int16_t* pambient_dmax_mm)
{
    ...
    status = VL53LX_hist_ambient_dmax(target_reflectance, pdmax_cal, pdmax_cfg, pbins, pambient_dmax_mm);

    return status;
}


VL53LX_Error VL53LX_ipp_xtalk_calibration_process_data(VL53LX_DEV Dev,
                                                       ...
                                                       VL53LX_xtalk_calibration_results_t* pxtalk_cal)
{
    ...
    status = VL53LX_xtalk_calibration_process_data(pxtalk_ranges, pxtalk_shape, pxtalk_cal);
